...
import { TimeBlock, CATEGORY_COLOR_MAP } from "@/types/dayflow";
            ...
            <div className="p-4 space-y-4">
              ...
              <div>
                <label className="text-xs font-medium text-muted-foreground mb-1 block">
                  <Clock className="inline h-3.5 w-3.5 mr-1" />Start Time
                </label>
                <div className="flex items-center gap-3">
                  <button onClick={() => adjustStart(-0.25)} className="p-1.5 rounded-lg border border-border active:bg-secondary">
                    <Minus className="h-4 w-4" />
                  </button>
                  <span className="text-sm font-medium min-w-[80px] text-center">{formatHour(startHour)}</span>
                  <button onClick={() => adjustStart(0.25)} className="p-1.5 rounded-lg border border-border active:bg-secondary">
              ...
              <div className="text-xs text-muted-foreground">
                <p>Date: {block.date} &middot; Type: {block.type}</p>
              </div>
